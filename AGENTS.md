...
This is the `Caplord/ImageSlideshow` fork, an iOS 15+ UIKit image-slideshow library. It ships a core Swift package product plus separate AlamofireImage, SDWebImage, and Kingfisher integration products. Keep the public API source-compatible unless the change is intentionally versioned as breaking.
...
- `ImageSlideshow.podspec` describes the CocoaPods distribution. Keep SPM and podspec source/dependency declarations aligned.
- `Example/project.yml` generates the gitignored example `.xcodeproj`; `Example/Podfile` integrates the local pod and its subspecs.
...
- Build all SPM products for iOS as CI does: temporarily remove the tracked `_Pods.xcodeproj` symlink, run `xcodebuild -scheme ImageSlideshow-Package -destination 'generic/platform=iOS Simulator' build`, then restore the symlink before committing.
- Prepare the example: `cd Example && xcodegen generate && pod install`.
- Test the example from `Example/`: `xcodebuild test -workspace ImageSlideshow.xcworkspace -scheme ImageSlideshow-Example -destination 'platform=iOS Simulator,name=<available iPhone>' CODE_SIGNING_ALLOWED=NO`.
- After package or podspec changes, build the package products and the example because the two distribution paths include different adapters/resources.
...
- Never edit `Example/ImageSlideshow.xcodeproj/project.pbxproj` directly. Change `Example/project.yml`, regenerate with `xcodegen generate`, then run `pod install`; likewise, never edit the consuming app's generated `project.pbxproj`.
- Treat `.build/`, `Example/Pods/`, and DerivedData as generated. Do not commit them. `_Pods.xcodeproj` is a tracked compatibility symlink, so restore it after local SPM builds.
...
- Update `CHANGELOG.md`, `README.md`, `Package.swift`, and the podspec together when a release changes supported integrations or requirements. Do not tag or publish without explicit authorization.
